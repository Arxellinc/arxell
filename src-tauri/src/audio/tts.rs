...
impl KokoroDaemon {
    ...
    pub fn speak(&mut self, text: &str, voice: Option<&str>) -> Result<SpeakResult, String> {
        ...
        Ok(SpeakResult { audio: audio_buf, phonemes })
    }
...
pub fn known_kokoro_voices() -> Vec<String> {
    vec![
        ...
        "jf_alpha", "jf_gongitsune", "jf_nezumi", "jf_tebukuro", "jm_kumo", "zf_xiaobei",
        "zf_xiaoni", "zf_xiaoxiao", "zf_xiaoyi", "zm_yunjian", "zm_yunxi", "zm_yunxia",
        "zm_yunyang", "ef_dora", "em_alex", "em_santa", "ff_siwis", "hf_alpha", "hf_beta",
        "if_sara", "if_nicola", "pf_dora", "pm_alex", "pm_santa",
    ]
    .into_iter()
    .map(|s| s.to_string())
    .collect()
