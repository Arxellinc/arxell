...
impl Agent {
    pub fn new(
        ...
        let cwd = cwd.unwrap_or_else(|| std::env::current_dir().unwrap_or_default().display().to_string());
    ...
    pub async fn run_collect(
        ...
        while turn < max_turns {
            ...
            let did_compact = self.check_compaction(&mut events, stop_reason, cancel.clone()).await;
            if did_compact {
