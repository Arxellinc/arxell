...
impl Session {
    ...
    pub fn get_sessions_dir(cwd: &str) -> PathBuf {
        ...
        let safe = cwd.replace('/', "-").replace('\\', "-").trim_matches('-').to_string();
        let dir = PathBuf::from(home).join(CONFIG_DIR_NAME).join("sessions").join(safe);
    ...
    pub fn create(
        ...
        if persist {
            ...
            s.session_file = Some(Self::get_sessions_dir(&cwd).join(format!("{}_{}.jsonl", file_ts, s.id)));
        }
    ...
    fn persist_entry(&mut self) -> KonResult<()> {
        ...
        let has_assistant = self.entries.iter().any(|e| {
            matches!(e, SessionEntry::Message { message: Message::Assistant { .. }, .. })
        });
        if !has_assistant {
            return Ok(());
        }
    ...
    pub fn append_model_change(&mut self, provider: String, model_id: String, base_url: Option<String>) -> KonResult<String> {
    ...
    pub fn load(path: impl AsRef<Path>) -> KonResult<Self> {
        ...
        let header = header.ok_or_else(|| KonError::InvalidArgument(format!("invalid session file: {}", path.display())))?;
    ...
    pub fn set_model(&mut self, provider: String, model_id: String, base_url: Option<String>) -> KonResult<()> {
        if self
            .model()
            .as_ref()
