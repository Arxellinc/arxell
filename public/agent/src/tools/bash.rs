...
impl Tool for BashTool {
    ...
    async fn execute(&self, params: Value, _cancel: Option<tokio::sync::watch::Receiver<bool>>) -> ToolResult {
        ...
        let out = match tokio::time::timeout(std::time::Duration::from_secs(timeout), cmd.output()).await {
            Ok(Ok(o)) => o,
        ...
        ToolResult {
            ...
            result: Some(if compact.is_empty() { "(no output)".to_string() } else { compact.clone() }),
            images: None,
