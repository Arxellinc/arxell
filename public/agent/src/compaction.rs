...
pub fn is_overflow(usage: &Usage, context_window: i64, max_output_tokens: i64, buffer_tokens: i64) -> bool {
    let total = usage.input_tokens + usage.output_tokens + usage.cache_read_tokens + usage.cache_write_tokens;
...
pub async fn generate_summary(
    ...
    let prompt = Message::User {
        content: crate::types::UserContent::Text(
            "Summarize this conversation preserving key decisions, files, and pending work.".to_string(),
        ),
    };
    ...
    let mut stream = provider
        .stream(msgs, Some(system_prompt), Some(Vec::<ToolDefinition>::new()), None, Some(1024))
        .await?;
