...
fn convert_messages(messages: Vec<Message>, system_prompt: Option<String>) -> Vec<serde_json::Value> {
    let mut out = Vec::new();
    ...
    for msg in messages {
        match msg {
            Message::User { content } => match content {
                crate::types::UserContent::Text(t) => out.push(serde_json::json!({"role":"user","content":t})),
            ...
            Message::Assistant { content, .. } => {
                ...
                for p in content {
                    match p {
                        ...
                        crate::types::ContentPart::Thinking { thinking, .. } => text.push_str(&thinking),
                        _ => {}
                    }
                }
